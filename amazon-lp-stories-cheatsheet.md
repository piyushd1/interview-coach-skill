...
**Situation**: When I analyzed satisfaction data by acquisition channel, a striking pattern emerged: 28% of all JD Xperts leads were coming from users who had called directly. These were the most high-intent users we had — they'd already picked up the phone. But their satisfaction was 2.8–3.2 vs. 4.2 for online users, same-category repeat was below 5%, and most churned immediately. The CPA for this channel was effectively zero — we were already paying for these users. The problem: the call center ran on a legacy text-based console with a 52-second average call window. It couldn't render any modern booking interface. Call center agents could take a message and pass it along, but couldn't actually create a JD Xperts managed booking in the moment. Our highest-intent, lowest-CPA cohort was falling into a broken handoff and leaving.

**Task**: Get these 28% of callers into the JD Xperts managed experience — structured booking, quality service, proper follow-up — without rebuilding call center infrastructure (owned by a different team, 9–12 months with admin blockers out of our control).

**Action**:
1. **Mapped solution options and found the unlock.** Option A: full call center modernization — the architecturally correct path, but 9–12 months, separate team dependency, admin blockers. Channel stays broken for a year. Option B: translation layer — wrap the legacy console with APIs that convert between its XML format and our modern JSON OMS. Weeks, not months. The insight that made option B viable: callers don't need to complete the entire booking on the phone. The 52-second window captures intent and starts the order. Everything else — payment, confirmation, tracking — happens asynchronously via WhatsApp after the call. Phone call initiates; digital touchpoints complete. That insight changed the entire design.
2. **Built a headless booking engine with translation + anti-corruption layers.** Translation layer APIs converted between legacy XML and modern JSON OMS. The anti-corruption layer was critical — years of inconsistent legacy data models (varied order states, time formats, user ID schemas) would have leaked dirty data into our clean OMS without it. ACL normalized everything before it touched our systems. Idempotency layer: unique hash of caller's phone number + 5-minute time window. If the same hash arrived twice, returned the existing booking. Designed as a stateless hash — survives agent session resets on unstable connections. Minimal console front-end: just enough to capture service type, location, and time preference within 52 seconds.
3. **Designed an async user journey via messaging.** Order details passed to Xperts OMS → WhatsApp + SMS deep links triggered to caller's phone. Deep links were purely informational — dropped users into service details, payment, tracking pages. No login wall. For callers who weren't app users, forcing authentication would have caused drop-off. Redis short-lived cache maintained multi-turn booking state for agents who needed mid-call availability lookups.
