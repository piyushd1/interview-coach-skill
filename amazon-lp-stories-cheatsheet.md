...
| Dive Deep | S021 (Login bug: source × browser segmentation) | S024 (Notification payload inspection) | S026 (Cohort analysis found silent targeting bug) | S003 (7 failure types — each needing different tech) |
...
**Situation**: JustDial processed ~8.5 lakh (850K) searches daily. 11% returned zero results — roughly 93,000 failed searches every day. These weren't searches for things JD didn't offer. They were real customers with real spending intent who couldn't express their need in a way keyword search understood. Every failed search was a user who left without connecting to a vendor: no connection event, no revenue. The failure rate was growing as India came online — more diverse linguistic patterns arriving that keyword search couldn't handle.

I analyzed the 93K failures and found they broke into 7 distinct failure types, each requiring a different technical approach:
- **Type 1 (~25K/day)** — Language failures: phonetic misspellings ("plumbar"), Hinglish code-switching ("AC thik karo"), regional script input → solvable by embedding model alone (no LLM needed for high-confidence matches)
- **Type 2 (~20K/day)** — Symptom-first / free-text ("my kitchen tap is leaking") → LLM intent extraction → pgvector
- **Type 3 (~10K/day)** — Brand eponyms ("Aquaguard service" = water purifier repair) → LLM entity disambiguation
- **Type 4 (~7K/day)** — Service vs. retail ambiguity ("wedding lehenga" — tailor or product?) → LLM routing decision
- **Type 5 (~5K/day)** — Over-constrained long-tail ("24/7 female plumber HSR Layout under ₹300") → LLM constraint relaxation
- **Type 6 (~17K/day)** — Hyperlocal supply gaps: valid search, zero vendors in that pin code → **NOT solvable by search tech** — fed directly into LPB metric (S005) to guide vendor recruitment
- **Type 7 (~9K/day)** — Too-broad / ambiguous ("home service", "repair") → LLM generates clarifying WhatsApp options

Key architecture insight: the embedding model alone handles Type 1 (high cosine similarity ≥0.7 threshold). Only lower-confidence matches and Types 2-7 escalate to the LLM. This meant ~25K/day were rescued with zero LLM cost.

**Task**: Fix search failure permanently. Most searches happened on app. I owned the end-to-end solution and led a cross-functional team of 8 across product, engineering, and data.

**Action**:
1. **Evaluated 3 options with explicit rejection criteria**: (a) Rules-based dictionary expansion — handled English misspellings but dead end for Hinglish. Cannot enumerate infinite spelling variants. Rejected. (b) Google Vertex API — strong general quality but unit economics were broken: ₹2.1/query × 93K/day = ₹1.95L/day = **₹5.8cr/year**. AND general models handled Hinglish code-switching poorly. Rejected. (c) Fine-tune in-house model: ₹38L setup + ₹4L/month infra = **₹86L/year — 6.7x cheaper**. Full cost and quality control. Chosen. Before building, found an existing asset: a Whisper-based LLM pipeline already running for phone call transcript processing — repurposed its foundation rather than starting from scratch. Saved ~6–8 weeks.
...
3. **Architecture — tiered pipeline**: (a) When a query failed, an interceptor pushed it to async queue (fail-open at 250ms — app never hangs). (b) **Embedding model first**: query converted to 768-dim vector → pgvector cosine similarity against 5K service categories. Confidence ≥0.7 = direct match (handles Type 1 — ~25K/day with zero LLM cost). Confidence <0.7 = escalate to LLM. *Why 0.7?* Tested 0.5–0.9 on 10K labeled set; 0.7 optimized F1 — below that, false positives spiked (irrelevant suggestions); above that, recall dropped too much. (c) **Llama 3.2 for Types 2-7**: intent extraction, entity disambiguation, routing decisions, constraint relaxation, clarifying question generation. LLM output validated against category taxonomy whitelist — hallucinations discarded, user dropped to category browse. (d) Corrected results delivered via WhatsApp/SMS — "Looking for AC repair? Here are top vendors near you." **62% CTR** on these messages. CTR was also the proxy for model accuracy before formal evaluation. (e) TTL of 5 min on RabbitMQ messages — if not processed, dropped (user has moved on). TTL hop-counter of 1 prevents infinite retries. (f) High-CTR corrected keywords cached back into Elasticsearch — feedback loop improves base search.
...
**Result**: Search failure 11% → ~2%. ~49K searches rescued daily: ~25K via embedding model alone (Type 1), ~24K via LLM pipeline (Types 2-7). 17K/day Type 6 supply gaps fed directly into LPB metric (S005) to guide vendor recruitment — the system turned an unsolvable search problem into a supply intelligence signal. Via WhatsApp retargeting (62% CTR), generated ~55-60K high-intent leads daily from ~50K unique users. 14% conversion rate on rescued searches (vs. 0% previously). Turned previously dead search traffic into the highest-volume lead generation channel.
...
- **Architecture**: (1) Query Interceptor — monitors Elasticsearch, flags zero-result queries, pushes to RabbitMQ. Fail-open at 250ms — app never hangs. 5-min TTL on messages (user has moved on if >5 min). (2) **Embedding model first** — query converted to 768-dim vector, pgvector cosine similarity against 5K pre-computed category vectors. Confidence ≥0.7 → direct match (handles Type 1 — 25K/day with zero LLM inference cost). Confidence <0.7 → escalate to LLM. (3) Llama 3.2 for Types 2-7 — intent extraction, brand disambiguation, service/retail routing, constraint relaxation, clarifying question generation. Taxonomy whitelist hallucination guard — mismatched outputs discarded, user dropped to category browse. TTL hop-counter of 1 prevents retry loops. (4) WhatsApp/SMS delivery — 62% CTR. User has already bounced from app; WhatsApp pulls them back. CTR = model accuracy proxy before formal eval. (5) Feedback loop — high-CTR keywords cached into Elasticsearch, improving base search. (6) Model evolution — external API → hosted → in-house fine-tuned Llama 3.2 (₹86L/year vs ₹5.8cr/year Vertex — 6.7x cheaper).
- **Key Technical Decision**: In-house Llama 3.2 fine-tuning vs. Google Vertex AI. Vertex: ₹2.1/query × 93K/day = ₹5.8cr/year. In-house: ₹38L setup + ₹4L/month = ₹86L/year. 6.7x cheaper AND better Hinglish quality (general models not trained on Indian local search patterns). Accepted 3-week quality regression during fine-tuning transition. Second key decision: pgvector over Elasticsearch — already on Postgres, 5K categories = tiny corpus, <50ms at this scale, zero new infra.
- **Scale**: Processing ~93K failed queries/day. ~49K rescued: 25K by embedding alone, 24K by LLM. 17K Type 6 supply gaps fed to LPB metric. 8-person cross-functional team (2 ML engineers, 3 backend, 1 data analyst, 1 QA, PM).

**LP Flex**:
- **Customer Obsession**: Lead with "93,000 searches failing daily — each one a high-intent customer we were losing. We turned them into leads instead."
- **Invent and Simplify**: Lead with "Two-tier architecture: embedding model handles 25K/day with zero LLM cost; LLM only fires for the harder 51K. Repurposed existing Whisper pipeline as foundation."
- **Dive Deep**: Lead with "93K daily failures broke into 7 distinct types — each needing different tech. Type 6 (supply gaps) wasn't a search problem at all — fed it into LPB metric instead."
- **Learn and Be Curious**: Lead with "Self-taught LangChain and fine-tuning — no ML background, learned on the job to build what the team couldn't buy"
- **Deliver Results**: Lead with "Search failure 11%→2%, 49K searches rescued daily, 55-60K leads via WhatsApp at 62% CTR"
...
**Quick Revision Anchors**:
- Key phrases: "93K failing searches" | "7 failure types" | "embedding model alone handles Type 1 (≥0.7 threshold)" | "Type 6 feeds LPB metric — not a search problem" | "fail-open at 250ms" | "TTL hop-counter of 1" | "taxonomy whitelist guard"
- Metric anchors: 11%→2% failure rate | ~49K rescued/day (25K embeddings + 24K LLM) | 55-60K leads/day | 62% WhatsApp CTR | Vertex ₹5.8cr/year vs in-house ₹86L/year (6.7x) | 14% conversion on rescued searches
- Decision point: In-house fine-tuning (₹86L/yr) over Vertex (₹5.8cr/yr) — 6.7x cheaper + Hinglish quality. pgvector over Elasticsearch — already on Postgres, 5K categories, <50ms.
...
**Situation**: When I analyzed satisfaction data by acquisition channel, a striking pattern emerged: 28% of all JD Xperts leads were coming from callers — our highest-intent cohort (they'd already picked up the phone). But CSAT was 2.8 vs. 4.2 for online users. Same-category repeat below 5%. CPA effectively zero. We were already paying for these users and failing them completely.

The obvious answer was: build a call center booking app. The problem was the call center infrastructure. JD operated centralized call centers where agents ran an old version of Firefox because the existing CRM — their primary tool for logging calls, looking up vendor info, and tracking dispositions — only worked reliably on that version. Installing new software on these machines required central IT approval: months of security reviews, compliance checks, staged rollout across offices. **The constraint wasn't technical capability — it was organizational velocity.** The full "right" solution would take 4-6 months of IT bureaucracy while our highest-intent cohort bled away daily.

**Task**: Get 28% of callers into the JD Xperts managed experience without installing anything on agent machines. No IT approvals. No changes to agent workflow. Weeks, not months.

**Action**:
1. **Found the unlock: the CRM already had HTTP webhooks.** The existing CRM fired an HTTP POST to its own logging server every time an agent completed a call disposition. I didn't need to install anything on agent machines or modify the CRM application. I added the translation layer as a **second recipient of that same webhook** — either by updating one line in the logging server's config (approved by IT in days, not months) or by putting a thin proxy in front of their logging endpoint that forwarded to both their logger AND my translation layer. Zero software on agent machines. Zero changes to the agent's workflow. The agents didn't even know the architecture changed. The translation layer mapped the CRM's fields to my OMS API:
```
CRM sends: caller_phone, category_selected (agent dropdown), location, agent_notes
My API receives: customer_phone, service_id (mapped from CRM codes), 
  location (geocoded from place name), preferred_time (extracted from notes)
```
2. **Anti-corruption layer + idempotency.** The CRM's data had years of inconsistency — varied order states, time format mismatches, phone numbers formatted differently. The ACL normalized everything before it hit the OMS. Idempotency: hash of caller phone + 5-minute window. Same hash within 5 minutes = agent double-click or retry, return existing booking. After 5 minutes = intentional new booking. Stateless hash — survives agent session resets.
3. **Async WhatsApp journey with phone-as-authentication.** The phone call captures intent. Everything else — vendor selection, payment, confirmation — happens via WhatsApp within 30 seconds of hanging up (P95: 22 seconds). **No login wall.** The deep link was pre-authenticated using the caller's phone number. They literally just called from that phone — the number IS the authentication. No password, no OTP, no account creation. This removed the single biggest drop-off in the app flow (login = where 35% of users abandoned). Fallback chain: WhatsApp (85% of India smartphones) → SMS with mobile web link → agent manual panel (Firefox-compatible, <3% of calls).
4. **Headless API = channel-agnostic from day one.** The booking engine had no UI of its own — pure API logic. Any channel could consume it. Adding new channels required zero core changes:
```
Channel         Method                              Timeline
Call center CRM  Webhook redirect                   ✅ 3 weeks
WhatsApp bot     WhatsApp Business API → same API   2 weeks
IVR system       IVR vendor → same API              3 weeks
Partner website  REST API + auth token               1 week
New vertical     Category config only               Config change
```

**Result**: 48% daily order growth (135 → 200/day) within 3 weeks. Caller conversion: 42-44% vs. 23-24% app. Why 42% vs. 23%? Three reasons: (1) trust was pre-established on the phone call, (2) zero login friction (phone = auth), (3) momentum — WhatsApp arrives <30 seconds after hanging up, customer is still in "I need this fixed" mode. *Selection bias check:* we offered the same WhatsApp deep-link flow to app users who abandoned at checkout → their completion jumped from 23% to 31%. The WhatsApp handoff itself added ~8pp regardless of channel. The remaining gap (31% → 42%) is the intent selection bias of callers. CSAT: 2.8 → 4.5. Near-zero CPA — most profitable acquisition source on the platform.

**Earned Secret**: "The call center constraint looked like a technical problem — legacy system, old browser. It was actually an organizational velocity problem — IT approval timelines. The real insight was that I didn't need to touch their machines at all. The CRM already fired webhooks. I just became the second recipient. The agents never knew anything changed. And because the booking engine was headless, every new channel after that took days to weeks instead of months."

**What I Actually Built**:
- **System/Service**: Headless booking engine — webhook redirect from legacy CRM to translation layer, with async WhatsApp completion journey using phone-as-authentication
- **Tech Stack**: Translation layer (webhook recipient, field mapper, geocoder, agent notes NLP), anti-corruption layer, Redis (idempotency cache + multi-turn booking state), WhatsApp Business API + SMS fallback, geocoding API
- **Architecture**: (1) Webhook redirect — CRM fires HTTP POST on call disposition to its own logging server; translation layer becomes a second recipient of that webhook (one server-side config change, approved by IT in days). Zero software on agent machines. Zero CRM changes. Agents never knew the architecture changed. (2) Field mapping — CRM fields (category dropdown code, location text, agent notes) → OMS fields (service_id, geocoded coordinates, parsed preferred_time). (3) ACL normalizes legacy data inconsistencies: order state labels, time formats, phone number formats. (4) Idempotency — phone + 5-min window hash in Redis: same hash = existing booking returned; after 5 min = intentional new booking. (5) OMS creates booking → triggers WhatsApp deep link within P95 22 seconds. Phone number = authentication on deep link (caller literally just called from that phone — no OTP, no login, no account needed). (6) Fallback chain: WhatsApp → SMS + mobile web → agent manual panel (<3% of calls, Firefox-compatible). (7) Headless API: no UI, any channel consumes it — call center was just the first.
- **Key Technical Decision**: Webhook redirect (IT approved in days) over call center app (4-6 months of IT bureaucracy). The constraint was organizational velocity, not technical capability. Second: phone-as-auth on WhatsApp — removed the 35% drop-off at login that plagued the app flow. Third: headless API — call center was channel 1; WhatsApp bot (2 weeks) was channel 2 using the same API.
- **Scale**: 135→200 orders/day (48% growth) in 3 weeks. 42-44% conversion vs. 23-24% app. WhatsApp bot added in 2 weeks using same API. New Xperts verticals = config change only (no rebuilds).

**LP Flex**:
- **Invent and Simplify**: Lead with "The CRM already had webhooks. I became the second recipient. Zero software on agent machines, zero changes to their workflow — the entire integration was invisible to them."
- **Bias for Action**: Lead with "28% of leads were callers at near-zero CPA with 2x conversion. The full fix was 4-6 months of IT approvals. I shipped the webhook redirect in 3 weeks."
- **Think Big**: Lead with "The call center was channel 1. Because the API was headless, WhatsApp bot was channel 2 in 2 weeks, IVR in 3 weeks, partner integration in 1 week. The call center unlock was actually a platform decision."
- **Deliver Results**: Lead with "135 to 200 orders/day in 3 weeks. 42% caller conversion vs. 23% app. Near-zero CPA — most profitable acquisition source."
- **Frugality**: Lead with "One server-side config change — one line pointing the CRM's existing webhook at a second recipient. Zero software installed. Zero agent training needed. Zero IT escalation."
...
**Quick Revision Anchors**:
- Key phrases: "organizational velocity — not technical" | "CRM already had webhooks, I became the second recipient" | "zero software on agent machines, agents never knew it changed" | "phone = authentication, no OTP no login" | "headless API — call center was channel 1"
- Metric anchors: 48% order growth | 135→200 orders/day | 42-44% conversion vs 23-24% app | 8pp from WhatsApp handoff alone (selection bias controlled) | CSAT 2.8→4.5 | P95 22 sec WhatsApp delivery | WhatsApp bot added in 2 weeks using same API
- Decision points: Webhook redirect (days) over call center app (4-6 months IT approvals). Phone-as-auth over forced login (removed 35% app drop-off). Headless API = each new channel is config not code.
...
|--------|----------------|--------------|-------------|
| A | Mismatched service intent (AC install → AC repair vendor) | ~20K | YES — reroute |
| B | Location mismatch (caller in Whitefield, vendor covers Koramangala) | ~6.5K | YES — match local vendor |
| C | Brand/model mismatch (Samsung AC caller → LG-only vendor) | ~6.5K | YES — reroute |
| D | Job seekers ("Bhai koi kaam hai kya?") | ~8K | YES — route to JD Jobs |
| E | Negative economic qualification (₹150 job, vendor 15km away) | ~6.5K | PARTIAL — batch nearby |
| F | B2B volume mismatch (bulk order to individual vendor) | ~3K | YES — route to JD Mart |
| G | Post-sales support ("you repaired my AC last week, problem is back") | ~4K | YES — follow-up ticket |
| H | Product vs. service confusion (wants to BUY a pipe fitting, not hire plumber) | ~2.5K | YES — route to JD Shopping |
...
2. **NER (Named Entity Recognition) pipeline.** Extracts structured entities from transcript: service type, brand, location, intent signals ("kaam chahiye" vs. "koi kaam hai kya"), price mentions, follow-up signals ("last week aaye the"). Turns unstructured audio into searchable data.
...
- **Deliver Results**: Lead with "30-35K new service leads/day from zero, ₹2.1cr/month revenue, 20x GPU cost ROI. Vendor spam rate dropped 80K→55K/day — virtuous cycle."

**EMXO Connection**: Voice-first emerging market insight. In India, a massive share of customer intent is expressed via phone calls, not digital forms. Building AI to extract and route voice intent is a capability directly applicable to EMXO markets.
**Data constraint angle**: No third-party data — extracted everything from first-party audio. Built from signals the platform already had but was discarding.
