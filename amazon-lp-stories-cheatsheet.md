...
**Situation**: JustDial's core product promise was "find anything local." Search was the product. When 11% of searches failed — 1 to 1.5 lakh searches every day — we weren't just delivering bad UX. We were breaking that promise at scale. Every failed search was a user who left without connecting to a vendor: no connection event, no revenue. The failure rate was growing, not shrinking — as India came online, more diverse linguistic patterns were arriving that our keyword-based Elasticsearch index couldn't handle. Of the 11% failures: 3% were true failures (zero results), 8% returned poor-quality results. In 60% of the 8% poor-result cases, users searched again with different terms; the remaining 40% fell back to broad category browsing. Failures clustered into 4 buckets: misspellings, colloquial/local language spellings, free-text natural language queries, and Hindi/Hinglish code-switching ("bijli ka kaam near me"). The last bucket was the hardest — the space of valid Hinglish spellings is essentially unbounded.
...
**Action**:
1. **Evaluated 3 options with explicit rejection criteria**: (a) Rules-based dictionary expansion — handled English misspellings but dead end for Hinglish. Cannot enumerate infinite spelling variants. Rejected. (b) Google Vertex API — strong general quality but two problems: unit economics were broken (at 1L+ daily failures, API costs would run ₹20–30L/year) AND general models handled Hinglish code-switching poorly because they're trained on clean internet text, not Indian local search patterns. Rejected. (c) Fine-tune in-house model — higher upfront cost, but full cost and quality control. Chosen. Before building, found an existing asset: a Whisper-based LLM pipeline already running for phone call transcript processing — repurposed its foundation rather than starting from scratch. Saved ~6–8 weeks.
